...
Le fond de carte CARTO demande une clé (`CARTO_API_KEY` dans `.env`) : sans elle, un filigrane « API KEY REQUIRED » apparaît. Cette clé reste **côté serveur** : le backend sert les tuiles via `/api/tiles/<z>/<x>/<y>.png` (proxy), le navigateur ne la voit jamais et elle n'est écrite dans aucune carte générée. Sur Render, définir `CARTO_API_KEY` sur le service backend.
...
5.  **`clean_immo.py`**
    * Le cœur du calcul, en 7 étapes internes séquentielles :
      ...
      0 bis. **Master = dernier run seulement + archive** — `master_immo_final.csv` ne garde que les annonces `active`, revues au dernier run de leur (ville, site) et d'un site actif (`sites_actifs` dans `scripts/scraping_config.json` : Vizzit, Century 21, Orpi ; c'est aussi la valeur par défaut de `data_fusion.py --sites`). Toutes les autres (anciennes, non revues, `inactive`, sites non actifs) sortent vers `backend/data/master_archive.csv` : historique cumulatif (une ligne par `url` + `prix` + `date_dernier_scan`, colonne `archive_le`) destiné au suivi des prix (tickets ORA-181, ORA-182). `annonces.db` reçoit ces lignes avec le statut `a_verifier`/`inactive`. Le modèle s'entraîne sur le master seul.
