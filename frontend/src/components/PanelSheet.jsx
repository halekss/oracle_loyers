...
export default function PanelSheet({ id, overline, title, actions, children }) {
  return (
    <section
      id={id}
      aria-label={title}
      className="flex-1 min-w-0 min-h-0 flex flex-col m-[14px] ml-0 rounded-[0_18px_18px_18px] border border-ink-800 bg-ink-900 overflow-hidden"
    >
      {(overline || title || actions) && (
        <header className="shrink-0 flex items-start justify-between gap-3 px-4 md:px-5 py-3.5 border-b" style={{ borderColor: '#1C2440' }}>
          <div className="min-w-0">
            {overline && (
              <p className="text-[10px] font-bold uppercase tracking-widest truncate text-accent-light">
                {overline}
              </p>
            )}
